...
export default function Officers() {
        ...
        Swal.fire({
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    http.delete(`delete-officers/${id}`)
                        .then((res) => {
                            setUpdate(!update);
                            setIsOpen(false);
                            Swal.fire({
    ...
    const submitData = (e) => {
        ...
        formData.append('bcs_batch', slideData.bcs_batch);
        if (slideData.id) {
            http.put(`update-officers/${slideData.id}`, formData)
                .then((res) => {
                    setUpdate(!update);
                    setIsOpen(false);
            ...
            http.post('save-officers', formData)
                .then((res) => {
                    setUpdate(!update);
                    setIsOpen(false);
                    setSlideData({ image: "", date: moment().format('YYYY-MM-DD') });
                    ...
                    <div className="page-content">
                        ...
                        <div className=" p-3">
                            <form onSubmit={submitData}>
                                ...
                                <input onChange={handleChange} name='bcs_batch' value={slideData.bcs_batch} type="text" className="form-control form-control-sm my-2"  placeholder="BCS Batch" />
