...
export default function AdministrationSubPage() {
        ...
        Swal.fire({
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    http.delete(`delete-administration-sub-pages/${id}`)
                        .then((res) => {
                            setUpdate(!update);
                            setIsOpen(false);
                            Swal.fire({
    ...
    const editData = (id) => {
        http.get(`administration-sub-pages/${id}`)
            .then((res) => {
    ...
    const submitData = (e) => {
        e.preventDefault()
        const formData = {
            pageId: Number(categoryData.pageId),
        ...
        if (categoryData.id) {
            http.put(`update-administration-sub-pages/${categoryData.id}`, formData)
                .then((res) => {
                    setUpdate(!update);
                    setIsOpen(false);
            ...
            http.post('save-administration-sub-pages', formData)
                .then((res) => {
                    setUpdate(!update);
                    setIsOpen(false);
                    setContent("");
    ...
    return (
        <div className='page-content adjustment-type-table'>
            <div className="custom-card p-2 d-flex justify-content-between mb-2 align-items-center">
                <h6>Administration Sub Pages</h6>
                {
