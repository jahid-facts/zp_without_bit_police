...
export default function ImportantContent() {
        ...
        Swal.fire({
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    http.delete(`delete-important-data/${id}`)
                        .then((res) => {
                            setUpdate(!update);
                            setIsOpen(false);
                            Swal.fire({
    ...
    const submitData = (e) => {
        e.preventDefault()
        if (contentData.id) {
            http.put(`update-important-data/${contentData.id}`, contentData)
                .then((res) => {
                    setUpdate(!update);
                    setIsOpen(false);
            ...
            http.post('save-important-data', {title: contentData.title, headingId: Number(contentData.headingId), link: contentData.link})
                .then((res) => {
                    setUpdate(!update);
                    setIsOpen(false);
