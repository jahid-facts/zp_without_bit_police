...
export default function ActivitySlider() {
        ...
        Swal.fire({
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    http.delete(`delete-activity-slider/${id}`)
                        .then((res) => {
                            setUpdate(!update);
                            setIsOpen(false);
                            Swal.fire({
    ...
    const submitData = (e) => {
        ...
        if (activityData.id) {
            http.put(`update-activity-slider/${activityData.id}`, formData)
                .then((res) => {
                    setUpdate(!update);
                    setIsOpen(false);
                    setActivityData({ image: "", title: "", });
                    Swal.fire({
                        position: 'top-center',
                        icon: 'success',
                        title: 'Success !',
            ...
            http.post('save-activity-slider', formData)
                .then((res) => {
                    setUpdate(!update);
                    setIsOpen(false);
                    setActivityData({ image: "", title: "", });
    ...
    return (
        <div className='page-content adjustment-type-table'>
            <div className="custom-card p-2 d-flex justify-content-between mb-2 align-items-center">
                <h6>Activity Slider</h6>
                <div>
                    <button style={{ marginTop: "1px" }} onClick={addSlide} className='btn btn-sm btn-primary float-end'>Add Slide</button>
                </div>
            </div>
