...
export default function RecentActivity() {
        ...
        Swal.fire({
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    http.delete(`delete-recent-activity/${id}`)
                        .then((res) => {
                            setUpdate(!update);
                            setIsOpen(false);
                            Swal.fire({
    ...
    const submitData = (e) => {
        ...
        if (activityData.id) {
            http.put(`update-recent-activity/${activityData.id}`, formData)
                .then((res) => {
                    setUpdate(!update);
                    setIsOpen(false);
            ...
            http.post('save-recent-activity', formData)
                .then((res) => {
                    setUpdate(!update);
                    setIsOpen(false);
                    setActivityData({ image: "", heading: "", content: "" });
        ...
        <div className='page-content adjustment-type-table'>
            <div className="custom-card p-2 d-flex justify-content-between mb-2 align-items-center">
                <h6>Recent Activity</h6>
                {
                    ...
                    <div>
                        <button style={{ marginTop: "1px" }} onClick={addActivity} className='btn btn-sm btn-primary float-end'>Add Page</button>
                    </div>
                }
            </div>
                    ...
                    <div className="page-content">
                        ...
                        <div className=" p-3">
                            <form onSubmit={submitData}>
                                ...
                                <textarea required className="form-control form-control-sm my-2" value={activityData.heading} name="heading" onChange={handleChange} id="" cols="30" rows="3" placeholder="Description"></textarea>
                                {
                                    activityData.id ?
