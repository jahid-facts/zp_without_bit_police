...
import { getTimeDifferenceInMinutes } from "../../../components/timeAndDate";
export default function BitPoliceNewsSingle() {
  ...
  const deleteRowData = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        http.delete(`delete-bit-news/${id}`).then((res) => {
          setUpdate(!update);
          setIsOpen(false);
          Swal.fire({
            position: "top-center",
  ...
  const columns = [
    ...
    {
      title: "Bit Officer",
      render: (row) => <div>{row?.officer?.name}</div>,
      cellStyle: {
        textAlign: "center",
      },
    },
...

    {
      ...
      field: "patient",
      render: (row) =>
        getTimeDifferenceInMinutes(row.createdAt) < 1440 && (
          <div>
            <button
  ...
  const submitData = (e) => {
    ...
    if (categoryData.id) {
      http
        .put(`update-bit-news/${categoryData.id}`, formData)
        .then((res) => {
          setUpdate(!update);
          setIsOpen(false);
