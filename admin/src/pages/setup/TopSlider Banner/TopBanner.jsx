...
export default function TopBanner() {
    ...
    const editData = (id) => {
        http.get(`banner/${id}`)
            .then((res) => {
                setBannerData({ ...res.data, date: moment().format('YYYY-MM-DD') });
                setIsOpen(true);
            })
            .catch((err) => {
                console.log(err)
    ...
    const handleChange = (e) => {
        setBannerData({ ...bannerData, [e.target.name]: e.target.files[0] });
    }
    const submitData = (e) => {
        e.preventDefault()
        ...
        if (bannerData.id) {
            http.put(`update-banner/${bannerData.id}`, formData)
                .then((res) => {
                    setUpdate(!update);
                    setIsOpen(false);
                    setBannerData({ image: "", date: moment().format('YYYY-MM-DD') });
                    Swal.fire({
                        position: 'top-center',
                        icon: 'success',
                        title: 'Success !',
            ...
            http.post('save-banner', formData)
                .then((res) => {
                    setUpdate(!update);
                    setIsOpen(false);
                    setBannerData({ image: "", date: moment().format('YYYY-MM-DD') });
        ...
        <div className='page-content adjustment-type-table'>
            <div className="custom-card p-2 d-flex justify-content-between mb-2 align-items-center">
                <h6>Top Banner</h6>
                {
                    ...
                    <div>
                        <button style={{ marginTop: "1px" }} onClick={addBanner} className='btn btn-sm btn-primary float-end'>Add Banner</button>
                    </div>
                }
                ...
                <div className="product_modal">
                    <div className="page-content">
                        <div className=" patients-head ">
                            <h5 className="fw-normal custom_py-3 px-2  text-start mb-2 card-name">Add Banner
                        ...
                        <div className=" p-3">
                            <form onSubmit={submitData}>
                                {/* <input onChange={handleChange} name='name' value={bannerData.name} type="text" className="form-control form-control-sm my-2" required placeholder="News" /> */}
