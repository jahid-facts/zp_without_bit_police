...
exports.getForces = async (req, res) => {
  const force = await prisma.Unit_Force.findMany({
    include: {
      unit: true,
      sub_units: true
    }
...
exports.getSingleForces = async (req, res) => {
  const force = await prisma.Unit_Force.findUnique({
    where: {
...
exports.saveForces = async (req, res) => {
  upload(req, res, async function (err) {
    ...
    const { name, designation, current_designation, mobile, phone, bcs_batch, sub_unitId, unitId } = req.body;
    const image = req?.file?.filename ? req?.file?.filename : null;

    try {
      const force = await prisma.Unit_Force.create({
        data: {
          name, designation, current_designation, mobile, phone, bcs_batch, sub_unitId: parseInt(sub_unitId), unitId: parseInt(unitId),
          image,
        },
      });
      res.json(force);
...
exports.updateForces = async (req, res) => {
  ...
  try {
    const existingForces = await prisma.Unit_Force.findUnique({ where: { id: forceId } });
    ...
    upload(req, res, async function (err) {
      ...
      const { name, designation, current_designation, mobile, phone, bcs_batch, sub_unitId, unitId, } = req.body;
      const updatedForces = await prisma.Unit_Force.update({
        where: { id: forceId },
        data: {
          name, designation, current_designation, mobile, phone, bcs_batch, sub_unitId: parseInt(sub_unitId), unitId: parseInt(unitId),
...
exports.deleteForces = async (req, res) => {
  const force = await prisma.Unit_Force.delete({
    where: {
      id: Number(req.params.id),
    }
  })
