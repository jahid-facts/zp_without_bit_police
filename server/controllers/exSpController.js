...
exports.getExSp = async (req, res) => {
  const officer = await prisma.ExSp.findMany({
    select: {
      id:true,
      name : true,
      image : true,
...
exports.getSingleExSp = async (req, res) => {
  const officer = await prisma.ExSp.findUnique({
    where: {
...
exports.saveExSp = async (req, res) => {
  upload(req, res, async function (err) {
    ...
    const { name, title, from_date, to_date, designationId } = req.body;
    const image = req?.file?.filename ? req?.file?.filename : null;

    try {
      const officer = await prisma.ExSp.create({
        data: {
...
exports.updateExSp = async (req, res) => {
  ...
  try {
    const existingExSp = await prisma.ExSp.findUnique({ where: { id: officerId } });
    ...
    upload(req, res, async function (err) {
      ...
      const updatedExSp = await prisma.ExSp.update({
        where: { id: officerId },
        data: {
...
exports.deleteExSp = async (req, res) => {
  const officer = await prisma.ExSp.delete({
    where: {
      id: Number(req.params.id),
    }
  })
